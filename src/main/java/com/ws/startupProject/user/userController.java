...
public class userController {
    ...
    GenericMessage createUser(@Valid @RequestBody UserCreate user) {
        ...
        String message = messageSource.getMessage("startupProject.messages.newUser", null, LocaleContextHolder.getLocale());
    ...
    ApiError handleActivationNotificationException(ActivationNotificationException exception) {
        ApiError apiError = new ApiError();
        apiError.setPath("/api/v1/users");
        String message = messageSource.getMessage("startupProject.messages.create.email.failure", null, LocaleContextHolder.getLocale());
        apiError.setMessage(message);
